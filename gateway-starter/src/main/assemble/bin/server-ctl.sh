...
REMOTE_TEST="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=${REMOTE_TEST_PORT}"
...
START()
{
  ...
  nohup java $JAVA_OPTS $MEM_OPTS $GCLOG_OPTS $JMX_OPTS $REMOTE_TEST $CRASH_OPTS $OTHER_OPTS $ADDITIONAL_OPTS -jar $BASEDIR/${APP_NAME}*.jar >> $LOGDIR/${APP_NAME}.out 2>&1 &
  PID=$!
  echo $PID > $PID_FILE
  sleep 3
